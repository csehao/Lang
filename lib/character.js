...
const { UseReveal, UsePoison, UseMed, UseHunt, UseGuard, UseKill, MedAndGuard } = require('./event');
...
class Judge{
    ...
    sendAction(action){
        ...
        if(action.constructor.name === 'UseMed'){
            const medTarget = this.characters[action.dst];
            const eventList = medTarget.eventList;
            for (let event of eventList) {
                if (event.constructor.name === 'UseKill') {
                    event.invalid();
                }
                if (event.constructor.name === 'UseGuard') {
                    this.sendAction(new MedAndGuard().setSource(action.src).setDestination(action.dst));
                }
            }
        }
        if(action.constructor.name === 'UseGuard'){
            const medTarget = this.characters[action.dst];
            const eventList = medTarget.eventList;
            for (let event of eventList) {
                if (event.constructor.name === 'UseKill') {
    ...
    resolveStatus(){
        ...
        for(var index in this.characters){
            ...
            if(status['UseKill'] && !(status['UseKill'].invalidated)){
                character.setDeath(true);
            } 
